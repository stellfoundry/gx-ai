...
__global__ void computeHermiteEnergySpectrum(cuComplex *m, float *hermite_spectrum, float *hermite_spectrum_avg, int count, int num_writes, int cutoff, int forcing_index) {
  int i = threadIdx.x;
  int idb = nx*nyc*nz*nl*i;

  // calculate |C_m| for each moment
  float value = m[idb + forcing_index].x*m[idb + forcing_index].x + m[idb + forcing_index].y*m[idb + forcing_index].y;

  hermite_spectrum[i] = value;
  ...
  if (count >= num_writes - cutoff) {
      ...
      hermite_spectrum_avg[i] = (index*hermite_spectrum_avg[i] + value)/(index + 1);
  }
}
...
{  
  ...
  cudaDeviceSynchronize();
  CUDA_DEBUG("Grad parallel periodic: %s/n");
  ...
  if (pars_->write_h_spectrum) {
    cudaMalloc((void **) &hermite_spectrum, sizeof(float)*pars_->nm_in);
    cudaMalloc((void **) &hermite_spectrum_avg, sizeof(float)*pars_->nm_in);
    cudaMallocHost((void **) &hermite_spectrum_h, sizeof(float)*pars_->nm_in);

    cudaMemset(hermite_spectrum, 0, pars_->nm_in*sizeof(float));
    cudaMemset(hermite_spectrum_avg, 0, pars_->nm_in*sizeof(float));

    sprintf(history_buffer, "%s.HermiteEnergySpectrumHistory.out", pars_->run_name);
    history_spectrum_file = fopen(history_buffer,"w+");
  }

    // need to define a temporary array for kz because kz is defined only as a function)
  //  for (int ikz=0; ikz<grids_->Nz; ikz++) {kpar[ikz] = kz(ikz);}

  // usage of ikp, ikpmax only makes sense if X0 and Y0 each are unity. BD
  //  int ikpmax = (int) (Nx-1)/3;
  //  for (int ikp=0; ikp<ikpmax; ikp++) {kperp[ikp] = ((float) ikp/ikpmax)*kpmax;} 
  //  int retval;
     
  //  if (retval = nc_put_var(id_->file, id_->kz,    grids_->kz_h))  ERR(retval); 
  //  if (retval = nc_put_var(id_->file, id_->kperp, grids_->ky_h)) ERR(retval); 

  /*
  if (driven) {
    if (retval = nc_put_var(id_->file, id_->nkstir, &nkstir)) ERR(retval);
    if (retval = nc_put_var(id_->file, id_->fampl,  &fampl))  ERR(retval);
    ...
    if (retval = nc_put_vara(id_->file, id_->kstir_x, s, c, &kstir_x[0])) ERR(retval);
    if (retval = nc_put_vara(id_->file, id_->kstir_y, s, c, &kstir_y[0])) ERR(retval);
    if (retval = nc_put_vara(id_->file, id_->kstir_z, s, c, &kstir_z[0])) ERR(retval);
  }
  */
...
Diagnostics::~Diagnostics()
{
  ...
  if (pars_->write_h_spectrum) {
    cudaFree(hermite_spectrum);
    cudaFree(hermite_spectrum_avg);
    cudaFreeHost(hermite_spectrum_h);
...
bool Diagnostics::loop_diagnostics(MomentsG* G, Fields* fields, float dt, int counter, float time) 
{
  ...
  if(pars_->write_omega) {
    ...
    if(counter%nw == 0) {
      cudaMemcpy(growth_rates_h, growth_rates, sizeof(cuDoubleComplex)*grids_->NxNyc, cudaMemcpyDeviceToHost);
  ...
  if(counter%nw == 0 && pars_->write_h_spectrum) {
...
void Diagnostics::writeMomOrFieldKpar(cuComplex* mom, const char* name) {
  ...
  CP_TO_CPU(amom_h,amom,sizeof(cuComplex)*Nx*(Ny/2+1)*Nz);
...
void Diagnostics::writeMomOrField(cuComplex* m, const char* name) {
  ...
  FILE* out = fopen(ofilename,"w+");
  CP_TO_CPU(amom_h,m,sizeof(cuComplex)*Nx*(Ny/2+1)*Nz);
...
{
  cuComplex val;
  CP_TO_CPU(&val, &f[iky + grids_->Nyc*ikx + grids_->NxNyc*iz], sizeof(cuComplex));
...
// set write_hermite_spectrum to "on" in gx_knobs to calculate and write to file the Hermite energy spectrum
void Diagnostics::HermiteEnergySpectrum(MomentsG *m, int count) {
    computeHermiteEnergySpectrum<<<1, pars_->nm_in>>>(m->G(0,0,0), hermite_spectrum, hermite_spectrum_avg, count/pars_->nwrite, pars_->nstep/pars_->nwrite, pars_->hermite_spectrum_avg_cutoff, pars_->forcing_index);
}
...
void Diagnostics::writeHermiteEnergySpectrum() {
  ...
  CP_TO_CPU(hermite_spectrum_h, hermite_spectrum_avg, sizeof(float)*pars_->nm_in);
  //  cudaMemcpy(hermite_spectrum_h, hermite_spectrum_avg, sizeof(float)*pars_->nm_in, cudaMemcpyDeviceToHost);

  fprintf(out, "#\tm (1)\t\t\tC_m,k (2)\t\t\t\n");  

  for (int i = 0; i < pars_->nm_in; i++) {
    fprintf(out, "\t%d\t%e\n", i, hermite_spectrum_h[i]);
  }
...
void Diagnostics::writeHermiteEnergySpectrumHistory() {
  ...
  CP_TO_CPU(hermite_spectrum_h, hermite_spectrum, sizeof(float)*pars_->nm_in);
  //  cudaMemcpy(hermite_spectrum_h, hermite_spectrum, sizeof(float)*pars_->nm_in, cudaMemcpyDeviceToHost);
...
void Diagnostics::close_netcdf_diag(NetCDF_ids* id)
{
  int retval;
