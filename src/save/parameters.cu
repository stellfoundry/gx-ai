...
int Parameters::read_namelist(char* filename)
{
  ...
  fnr_get_float(&namelist_struct, "gx", "epsl", &epsl);
  fnr_get_float(&namelist_struct, "gx", "kxfac", &kxfac);
  ...
  fnr_get_float(&namelist_struct, "gx", "tite", &ti_ov_te);   // ti_ov_te is overwritten if kinetic electrons
  ...
  write_h_spectrum        = get_bool_on_off(&namelist_struct, "gx", "write_h_spectrum");
  write_l_spectrum        = get_bool_on_off(&namelist_struct, "gx", "write_l_spectrum");
  write_lh_spectrum       = get_bool_on_off(&namelist_struct, "gx", "write_lh_spectrum");
  write_spec_v_time       = get_bool_on_off(&namelist_struct, "gx", "write_spec_v_time");
  write_moms              = get_bool_on_off(&namelist_struct, "gx", "write_moms");
  //  write_fields_kpar       = get_bool_on_off(&namelist_struct, "gx", "write_fields_kpar");
  write_fluxes            = get_bool_on_off(&namelist_struct, "gx", "write_fluxes");
  //  secondary_test          = get_bool_on_off(&namelist_struct, "gx", "secondary_test");
  //  nlpm_test               = get_bool_on_off(&namelist_struct, "gx", "nlpm_test");
  //  new_nlpm                = get_bool_on_off(&namelist_struct, "gx", "new_nlpm");
  //  nlpm_abs_sgn            = get_bool_on_off(&namelist_struct, "gx", "nlpm_abs_sgn");
  //  low_b                   = get_bool_on_off(&namelist_struct, "gx", "low_b");
  ...
  //  fnr_get_int(  &namelist_struct, "gx", "lh_iky", &lh_iky);
  //  fnr_get_int(  &namelist_struct, "gx", "lh_ikx", &lh_ikx);
  ...
  if(strcmp(initfield,"density") == 0)     { init = DENS;  }
  else if( strcmp(initfield,"phi") == 0)   { init = PHI;   }
  else if( strcmp(initfield,"force") == 0) { init = FORCE; }
  else if( strcmp(initfield,"tperp") == 0) { init = TPRP;  }
  else if( strcmp(initfield,"tpar") == 0)  { init = TPAR;  }
  else if( strcmp(initfield,"upar") == 0)  { init = UPAR;  }
  else if( strcmp(initfield,"odd") == 0)   { init = ODD;   }
  else if( strcmp(initfield,"RH_eq") == 0) { init = RH_equilibrium; new_varenna = true; }
  
  char* scheme_str;
  fnr_get_string(&namelist_struct, "gx", "scheme", &scheme_str);
  if( strcmp(scheme_str,"rk2") == 0) scheme = RK2;  
  if( strcmp(scheme_str,"rk3") == 0) scheme = RK3;  
  if( strcmp(scheme_str,"rk4") == 0) scheme = RK4;  
  if( strcmp(scheme_str,"k10") == 0) scheme = K10;  
  ...
  //  write_rh = get_bool_on_off(&namelist_struct, "gx", "write_rh");

  //  cuComplex phi_test;
  //  fnr_get_float(&namelist_struct, "gx", "phi_test_real", &phi_test.x);
  //  fnr_get_float(&namelist_struct, "gx", "phi_test_imag", &phi_test.y);
  //  fnr_get_int(  &namelist_struct, "gx", "iky_fixed", &iky_fixed);
  //  fnr_get_int(  &namelist_struct, "gx", "ikx_fixed", &ikx_fixed);

  //  fnr_get_string_no_test(&namelist_struct, "gx", "restartfile", &secondary_test_restartfileName);
  ...
  */
  ...
  /*
  fnr_get_float(&namelist_struct, "gx_nonlinear_terms_knobs", "densfac", &NLdensfac); if(NLdensfac!=0) NLdensfac = 1.;
  fnr_get_float(&namelist_struct, "gx_nonlinear_terms_knobs", "uparfac", &NLuparfac); if(NLuparfac!=0) NLuparfac = 1.;
  fnr_get_float(&namelist_struct, "gx_nonlinear_terms_knobs", "tparfac", &NLtparfac); if(NLtparfac!=0) NLtparfac = 1.;
  fnr_get_float(&namelist_struct, "gx_nonlinear_terms_knobs", "tprpfac", &NLtprpfac); if(NLtprpfac!=0) NLtprpfac = 1.;
  fnr_get_float(&namelist_struct, "gx_nonlinear_terms_knobs", "qparfac", &NLqparfac); if(NLqparfac!=0) NLqparfac = 1.;
  fnr_get_float(&namelist_struct, "gx_nonlinear_terms_knobs", "qprpfac", &NLqprpfac); if(NLqprpfac!=0) NLqprpfac = 1.;
  */
  ...
  // BD temporary hack for netcdf
  ion_z     = species_h[0].z;
  ion_mass  = species_h[0].mass;
  ion_dens  = species_h[0].dens;
  ion_fprim = species_h[0].fprim;
  ion_uprim = species_h[0].uprim;
  ion_temp  = species_h[0].temp; 
  ion_tprim = species_h[0].tprim; 
  ion_nu    = species_h[0].nu_ss;
