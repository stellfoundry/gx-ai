    ...
    if (idz < nz) {
      ...
      if (idlm < nm*nl) {
        ...
        if (unmasked(idx, idy)) {
          ...
          int m = idm + m_lo;
          cuComplex H_ = g[ig];
          if(m==0) H_ = H_ + zt_*Jflr(l, b_s)*phi_ + JflrB(l, b_s)*bpar_;
          if(m==1) H_ = H_ - zt_*vt_*Jflr(l, b_s)*apar_; 
          ...
          cuComplex Hcoll = -nu_ * nt_ * cuConjf(H_) * coll;
          cuComplex Hdrive = nt_ * cuConjf(H_) * drive;
