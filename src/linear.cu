...
{
  // estimate max linear frequency as ~ 0.5 c1 ( nz/(3 z0) ) ( ny/(3 y0) ) where c1 ~ 2 for ion_z = 1.

  omega_max[0] = 0.0; 
  omega_max[1] = 0.0;
  omega_max[2] = 0.5 * c1 * pow((float) grids_->Nz/3./pars_->z0, 0.5) * ((float) grids_->Ny/3./pars_->y0);
}
