...
void set_gryfxpars(struct gryfx_parameters_struct * gryfxpars, input_parameters_struct * pars);
...
// To be moved further inside program
/* 
...
//#endif
  ...
  gryfx->mpi.iproc = iproc;
  gryfx->mpi.mpcom = mpcom;
*/
...
//	set_grid_masks_and_unaliased_sizes(&(ev->grids));
  ...
  // copy elements of input_parameters struct inputs->pars into gryfx_parameters_struct gryfxpars
  if (iproc==0) set_gryfxpars(gryfxpars, inputs->getpars());
...
void set_gryfxpars(struct gryfx_parameters_struct * gryfxpars, input_parameters_struct * pars){
    gryfxpars->equilibrium_type = pars->equilibrium_type;
    ...
    if (pars->restart) gryfxpars->restart  = 1;
    ...
    gryfxpars->end_time = time(NULL) + pars->avail_cpu_time - pars->margin_cpu_time*1.2;
