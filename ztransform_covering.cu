//for each class
void ZTransformCovering(int nLinks, int nChains, int* ky, int* kx, cuComplex* f,cuComplex* result, cuComplex* g, float* kz_covering, char* abs, cufftHandle plan) {
  ...
  //coveringBounds<<<dimGridCovering,dimBlockCovering>>>(g, nLinks, nChains, ky);
  
    
  kzInitCovering<<<dimGridCovering,dimBlockCovering>>>(kz_covering, nLinks,NO_ZDERIV_COVERING);   //NO_ZDERIV is a bool that turns on or off ZDERIV terms
  //kz' = kz/nCoupled
  ...
  if(abs=="abs") {
    zderiv_abs_covering<<<dimGridCovering, dimBlockCovering>>> (g, nLinks, nChains, kz_covering);
  } else {
  ...
  coveringCopyBack<<<dimGridCovering,dimBlockCovering>>> (result, nLinks, nChains, ky, kx, g);
