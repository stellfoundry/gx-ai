...
!    zshift = real(2.0,kind(zshift))*xptsdum/ulim - real(1.0,kind(zshift))
  ...
  subroutine integrate_moment_gryfx (g, total_gryfx)
...
! <TT
    use theta_grid, only: ntgrid
    use mp, only: sum_reduce, sum_allreduce_sub, nproc, sum_allreduce, proc0

    implicit none

    complex, dimension (-ntgrid:,:,g_lo%llim_proc:), intent (in) :: g
    complex*8, dimension (g_lo%naky*g_lo%ntheta0*2*ntgrid*g_lo%nspec), intent (out) :: total_gryfx
    complex, dimension(:,:,:,:),allocatable :: total_small
    integer :: is, il, ie, ik, it, iglo, iz, ig, index_gryfx
  ...
  end subroutine integrate_moment_gryfx
