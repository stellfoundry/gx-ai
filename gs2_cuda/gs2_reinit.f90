...
!we want to check if dt_gryfx = 2*dt_gs2 is too big/small when compared to dt_cfl_gryfx
...
! If timestep is too big, make it smaller
    if (code_dt*2.0 > code_dt_cfl) reset = .true.
       
! If timestep is too small, make it bigger
    if (code_dt*2.0 < min(dt0, code_dt_cfl/delt_adj/delt_cushion)) reset = .true.
