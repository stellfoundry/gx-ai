...
int main(int argc, char* argv[])
{
	...
	fcndx = (cufftReal*) malloc(sizeof(cufftReal)*Nx*Ny);
	fcndy = (cufftReal*) malloc(sizeof(cufftReal)*Nx*Ny);
	...
	for(int j=0; j<Ny; j++) {
	  for(int i=0; i<Nx; i++) {
	    int index = i + Nx*j;
	    if(abs(dxcheck[index] - fcndx[index]) < .0001) { xequal = true;}
	    else {xequal = false; fprintf(ofile, "\n%d\n",index);}
	  }
	}    
	if(xequal == true) {fprintf(ofile, "\nDX CHECKS\n"); printf("DX CHECKS\n");}
	else {fprintf(ofile, "\nDX DOES NOT CHECK\n"); printf("DX DOES NOT CHECK\n");}
	
	bool yequal = false;
	for(int j=0; j<Ny; j++) {
	  for(int i=0; i<Nx; i++) {
	    int index = i + Nx*j;
	    if(abs(dycheck[index] - fcndy[index]) < .0001) { yequal = true;}
	    else {yequal = false; fprintf(ofile, "\n%d\n",index);}
	  }
	}    
	if(yequal == true) {fprintf(ofile, "\nDY CHECKS\n"); printf("DY CHECKS\n");}
	else {fprintf(ofile, "\nDY DOES NOT CHECK\n"); printf("DY DOES NOT CHECK\n");}
